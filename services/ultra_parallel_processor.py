...
def detect_hardware_encoders() -> List[str]:
    """Detect ALL available hardware encoders on this system."""
    ...
    try:
        ...
        for enc_name, enc_data in [
            ('h264_nvenc', {'name': 'nvenc', 'preset': 'p1', 'params': ['-rc', 'vbr', '-tune', 'hq', '-multipass', 'disabled']}),
            ('h264_vaapi', {'name': 'vaapi', 'preset': 'ultrafast', 'params': ['-vaapi_device', '/dev/dri/renderD128']}),
            ('h264_qsv', {'name': 'qsv', 'preset': 'veryfast', 'params': ['-global_quality', '23']}),
            ('h264_videotoolbox', {'name': 'videotoolbox', 'preset': 'ultrafast', 'params': ['-realtime', '1']}),
        ]:
            if enc_name in encoders_output:
                available.append(enc_data['name'])
                logger.info(f"Detected hardware encoder: {enc_name} ({enc_data['name']})")
...
class EncodeWorkerPool:
    ...
    def _build_ffmpeg_cmd(self, segment_path: str, quality: Dict,
        ...
        quality_dir = os.path.join(output_dir, quality_name, f'seg_{segment_index + 1:010d}')
    ...
    def _execute_encode(self, task: EncodeTask) -> QualityRenditionResult:
        ...
        try:
            ...
            # Read progress to prevent buffer deadlock
            ...
            if process.returncode != 0:
                ...
                # Check for NVENC-specific errors that indicate GPU overload
                if task.use_hardware and any(err in stderr.lower() for err in
                    ['out of memory', 'cuda', 'nvenc', 'no memory', 'unknown error',
                     'cannot load', 'driver', 'insufficient', 'buffer']):
                    logger.warning(f"GPU encode failed for {quality_name} seg {seg_idx}: "
                                  f"{stderr[:100]}... retrying with software")
                    # Retry with software
                    task.use_hardware = False
                    task.retry_count += 1
                    return self._execute_encode(task)
...
class HLSSegmentMerger:
    """Merges ALL segment results into final FULL-video HLS playlists."""
    ...
    def merge_quality_playlist(quality_name: str, output_dir: str,
        ...
        """Merge ALL segment playlists → final playlist containing FULL video."""
        final_quality_dir = os.path.join(output_dir, quality_name)
        os.makedirs(final_quality_dir, exist_ok=True)
        ...
        for seg_result in sorted_results:
            for ts_file in seg_result.segment_files:
                if os.path.exists(ts_file):
                    ...
                    new_path = os.path.join(final_quality_dir, new_name)
                    try:
        ...
        overall_playlist = os.path.join(final_quality_dir, f'{quality_name}.m3u8')
        ...
        if all_segments:
            first_seg = os.path.join(final_quality_dir, all_segments[0])
        ...
        return {
            'name': quality_name,
            'playlist': f'{quality_name}/{quality_name}.m3u8',
            'segments': total_segments,
            'bitrate': total_bitrate,
            'segment_files': [f'{quality_name}/{s}' for s in all_segments]
        }

    @staticmethod
    def generate_master_playlist(output_dir: str, quality_info: Dict[str, Dict],
                                  qualities: List[Dict]) -> str:
        """Generate master.m3u8 with ALL quality renditions."""
        ...
        with open(master_path, 'w') as f:
            ...
            for qi in sorted_qualities:
                ...
                playlist = qi.get('playlist', f'{qname}/{qname}.m3u8')
...
class UltraParallelProcessor:
    ...
    def process_video(self, input_path: str, video_id: int, output_dir: str = None,
        ...
        """Process FULL video. ALL segments. ALL qualities. NO skips."""
        if qualities is None:
            qualities = ULTRA_QUALITY_LADDER
        ...
        try:
            ...
            thumbnail = self.merger.generate_thumbnail(input_path, output_dir,
                width=info.get('width', 7680), height=info.get('height', 4320))
...
def process_batch_ultra(input_paths: List[str],
    ...
    max_workers = max_workers or min(8, max(1, len(input_paths)))
