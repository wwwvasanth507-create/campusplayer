...
class ParallelHLSTranscoder:
    ...
    def generate_quality_ladder_hls(self, input_path: str, output_dir: str, 
        ...
        qualities_to_encode = [q for q in QUALITY_LADDER if q['height'] <= max_height]
        if not qualities_to_encode:
            qualities_to_encode = [QUALITY_LADDER[0]]
        
        # Encode sequentually with ThreadPoolExecutor (reliable for 30hr+ videos)
        def encode_rendition(quality: Dict) -> Tuple[str, Dict, str]:
            """Encode a single rendition - runs in thread pool."""
            ...
            try:
                ...
                # Read stdout line by line (discard) to prevent memory buildup
                ...
                if process.returncode != 0:
                    ...
                    return rendition_name, {}, f"FFmpeg error: {stderr}"
        ...
        # Generate thumbnail from 720p or highest available
        thumbnail_path = ''
        for res in ['720p', '480p', '360p']:
            thumb_candidate = os.path.join(output_dir, res, 'thumbnail.jpg')
            if os.path.exists(thumb_candidate):
                thumbnail_path = f'{res}/thumbnail.jpg'
                break
...
def process_batch_videos(jobs: List[Dict], synth_dir: str = None):
    """
    Process a batch of videos: generate synthetic content, then transcode.
    Runs in background thread.
    For a single 25GB video with all HLS quality renditions.
    """
    ...
    # Process each job sequentially with parallel encodes
