...
class DeviceCapabilityDetector {
    ...
    static _qualityToHeight(quality) {
        const map = {
            ...
            '720p': 720, '1080p': 1080, '2K': 1440, '4K': 2160, '8K': 4320
        };
    ...
    static _determineMaxQuality(screen, cpu, memory) {
        ...
        // Downgrade based on CPU
        if (cpu.tier === 'low' && this._qualityToHeight(maxQ) > 720) {
            maxQ = '720p';
        } else if (cpu.tier === 'medium' && this._qualityToHeight(maxQ) > 1080) {
            maxQ = '1080p';
        }
        
        // Downgrade based on memory
        if (memory.tier === 'low' && this._qualityToHeight(maxQ) > 480) {
            maxQ = '480p';
...
const QUALITY_RANK = {
    ...
    '720p': 4, '1080p': 5, '2K': 6, '4K': 7, '8K': 8
};

const QUALITY_LABELS = {
    '144p': '144p', '240p': '240p', '360p': '360p', '480p': '480p',
    '720p': '720p', '1080p': '1080p', '2K': '2K', '4K': '4K', '8K': '8K'
};
...
class CampusPlayer {
    ...
    toggleFullscreen() {
        if (!document.fullscreenElement) {
            ...
            document.body.classList.add('campus-body-fullscreen');
            ...
            document.body.classList.remove('campus-body-fullscreen');
    ...
    onFullscreenChange() {
        ...
        this.player.classList.toggle('campus-fullscreen', this.state.isFullscreen);
        document.body.classList.toggle('campus-body-fullscreen', this.state.isFullscreen);

        if (!this.state.isFullscreen && screen.orientation && screen.orientation.unlock) {
            screen.orientation.unlock();
        }
    }
