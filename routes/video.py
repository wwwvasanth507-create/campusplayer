...
def _update_video_record(video_id: int, result: dict):
    ...
    try:
        ...
        if renditions:
            ...
            video.source_width = renditions[0].get('width', 7680) if renditions else 1920
            video.source_height = renditions[0].get('height', 4320) if renditions else 1080
            ...
            if result.get('success'):
                ...
                if hls_result.get('success'):
                    ...
                    hls_result = process_video_to_hls(
                        vid,
                        result['file_path'],
                        max_height=4320  # Max height for full quality ladder
                    )
                    if hls_result.get('success'):
                        video_obj = Video.query.get(vid)
                        if video_obj:
